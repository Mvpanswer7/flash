...
object Launcher extends Logging {
  ...
  def createEngine (rotorSession: RotorSession): SparkSession = {
    ...
    val store = rotorSession.engineConf.getOrElse("rotor.carbondata.store", "").toString
    if (rotorSession.engineConf.contains("rotor.carbondata.warehouse")) {
      val warehouse = rotorSession.engineConf.getOrElse("rotor.carbondata.warehouse", "").toString
      engineBuilder.config("spark.sql.warehouse.dir", warehouse)
    }
